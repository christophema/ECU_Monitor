...
void ADC0_vInit(void)
{
  ...
  ADC0_CHCTR8    =  0x1004;      // load channel control register
  ...
  ADC0_RCR0      =  0x0050;      // load result control register 0
  ...
  ADC0_RCR1      =  0x0050;      // load result control register 1
  ...
  ///  - the SR 0 line become activated if the event 9 interrupt is generated
  ...
  ADC0_CRMR1     =  0x0009;      // load conversion request mode register 1
  ...
  P5_DIDIS       =  0x0120;      // Port 5 Digital input disable register
...
//                ubSourceNum -> ADC0_SOURCE_1,indicates Parallel source
...
void ADC0_vClearPendingBits(ubyte ubSourceNum)
{
      ...
      if(ubSourceNum == 0) 
      {
         ADC0_QMR0 |= 0x0100; // Clear sequential source 0 pending bits 

      }
      else if(ubSourceNum == 1) 
      {
         ADC0_CRMR1 |= 0x0100; // Clear parallel source pending bits 

      }
      else
      {
         ADC0_QMR2 |= 0x0100; // Clear sequential source 2 pending bits 
...
void ADC0_vAliasChNum(ubyte ubAliasChNum, ubyte ubReqChNum)
{
      ...
      if(ubAliasChNum == 0) 
      {
         ADC0_ALR0 |= (0x000f) & (uword)ubReqChNum; // updates requested channel to ALIAS 0 bit field 

      }
      else if(ubAliasChNum == 1) 
      {
         ADC0_ALR0 |= (((0x000f) & (uword)ubReqChNum) << 8); // updates requested channel to ALIAS 1 bit field 
...
_interrupt(ADC0_SRN0INT)  void ADC0_viSRN0(void)
{
    ...
    if((ADC0_EVINFR & 0x0002) == 0x0002)    //Source1 event interrupt 
    {
       ADC0_EVINCR = 0x0002;     // Clear source1 event interrupt

      // USER CODE BEGIN (ADC0_viSRN0,2)
    ...
    if((ADC0_EVINFR & 0x0100) == 0x0100)    //Result0 event interrupt 
    {
      ...
      // USER CODE BEGIN (ADC0_viSRN0,20)
        EMSWaterOutletTempAcquisition();//安保采集淡水出机温度
      // USER CODE END
    ...
    if((ADC0_EVINFR & 0x0200) == 0x0200)    //Result1 event interrupt 
    {
      ...
      // USER CODE BEGIN (ADC0_viSRN0,21)
        ...
        EMSIntakeOilPressureAcquisition();//安保采集机油进机压力
      // USER CODE END
